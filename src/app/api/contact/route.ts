...
export async function POST(request: Request) {
  try {
    ...
    const emailSubject = body.role
      ? `Nova Proposta de Trabalho - ${body.role} - ${body.company}`
      : `Nova Solicitação de Contato - ${body.name}`;

    // Email para mim (desenvolvedor)
    ...
    // Email para o recrutador/cliente
    ...
    await Promise.all([
      transporter.sendMail(developerMailOptions),
